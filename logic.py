...
def ai_interpret_hexagram(
    ...
    if is_connectivity_test:
        ...
        system_prompt = (
            "你是一位精通《周易》的现代解卦专家，也是心理咨询与人生规划顾问。\n"
            "现在有一位用户通过三枚铜钱法起卦，已经得到本卦、动爻和（如有）变卦。\n"
            "请你在尊重《易经》原理的前提下，用现代人容易理解的语言，结合卦象、卦辞和爻辞，"
            "围绕用户的提问进行深入分析与建议。\n"
            "需要注意：\n"
            "1. 不要只给吉凶判断，要重点给出可操作的建议和提醒；\n"
            "2. 可以引用适量经典原文，但务必配上白话解释；\n"
            "3. 不要涉及迷信恐吓，不要给出医学、法律等刚性结论，只给方向性参考；\n"
            "4. 语言风格平和、理性、温暖；\n"
            "5. 不要输出思维链、推理过程、thinking process 或类似前置分析，"
            "直接给出最终解读即可。\n"
        )
        ...
        user_content = (
            ...
            "用户的具体占问如下：\n"
            f"{user_question}\n\n"
            "请根据以上信息，从整体卦象、动爻与变卦的关系来分析此事的趋势，"
            "并给出条理清晰的解读和建议。"
        )
